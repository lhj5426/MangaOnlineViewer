// ==UserScript==
// @name          Manga OnlineViewer Adult 260207AI魔改 跳页下载 配合强制下载用
// @author        Tago
    ...
    const options = {
          ...
          <div style="margin-top: 15px; padding: 15px; background: #fff3cd; border: 2px solid #ffc107; border-radius: 8px;">
            <div style="display: block;">
              ...
              <div style="display: flex; align-items: center; gap: 10px; justify-content: center; flex-wrap: wrap;">
                <label style="display: inline-block; padding: 8px 16px; background: #6c757d; color: white; border-radius: 4px; cursor: pointer; font-size: 13px; font-weight: bold; flex: 0 0 auto;">
                  选择文件
                  <input 
                    type="file" 
                ...
                <span id="fileNameDisplay" style="font-size: 13px; color: #856404; max-width: 350px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; flex: 1 1 auto; text-align: left; padding: 4px 8px; background: rgba(255,255,255,0.7); border-radius: 4px;">
                  未选择文件
                </span>
                <button
                  type="button"
                  id="clearImportBtn"
                  style="padding: 6px 15px; background: #dc3545; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 12px; display: none; flex: 0 0 auto; white-space: nowrap; font-weight: bold;"
                >
                  清除
                </button>
              </div>
              <div id="importStatus" style="font-size: 13px; color: #155724; text-align: center; display: none; margin-top: 10px; line-height: 1.6; font-weight: bold;">
              </div>
              <div style="font-size: 12px; color: #856404; text-align: center; font-style: italic; margin-top: 8px; line-height: 1.4;">
                💡 导入txt文件后，将只加载文件中列出的页面
              </div>
            </div>
          </div>
        </div>
      ...
      didOpen() {
        ...
        if (inputSlider) {
          ...
          const clearImportBtn = document.querySelector('#clearImportBtn');
          ...
          if (importInput) {
            importInput.addEventListener('change', function(e) {
                  ...
                  if (pageNumbers.length > 0) {
                    ...
                    if (importStatus) {
                      const rangeText = uniquePages.length <= 10 
                        ? uniquePages.join(', ')
                        : `${firstPage}...${lastPage} (共${uniquePages.length}页)`;
                      importStatus.innerHTML = `
                        <div style="color: #155724; font-weight: bold;">
                          ✓ 跳页加载模式已启用
                        </div>
                        <div style="color: #856404; margin-top: 4px;">
                          将只加载以下 ${uniquePages.length} 个页面: ${rangeText}
                        </div>
                      `;
                    ...
                    // 显示清除按钮
                    if (clearImportBtn) {
                      clearImportBtn.style.display = 'inline-block';
                    }
                    ...
                    logScript(`Skip-load mode enabled: ${uniquePages.length} pages`, uniquePages);
                  } else {
                    if (importStatus) {
                      importStatus.innerHTML = '<div style="color: #856404;">⚠ 未找到有效的页码</div>';
                      importStatus.style.display = 'block';
                    }
                  }
                } catch (error) {
                  logScript('Error parsing unloaded list:', error);
                  if (importStatus) {
                    importStatus.innerHTML = '<div style="color: #721c24;">✗ 文件解析失败</div>';
                    importStatus.style.display = 'block';
                  }
                }
              };
              ...
              reader.onerror = function() {
                if (importStatus) {
                  importStatus.innerHTML = '<div style="color: #721c24;">✗ 文件读取失败</div>';
                  importStatus.style.display = 'block';
                }
              };
            ...
            if (clearImportBtn) {
              clearImportBtn.addEventListener('click', function() {
                ...
                // 恢复输入框
                if (pageBeginInput) {
                  pageBeginInput.value = beginPage;
                  pageBeginInput.disabled = false;
                  pageBeginInput.style.background = '';
                }
                if (pageEndInput) {
                  pageEndInput.value = endPage;
                  pageEndInput.disabled = false;
                  pageEndInput.style.background = '';
                }
                
                // 重置滑块
                if (inputSlider && rangeSliderElement) {
                  rangeSliderElement.value([beginPage, endPage]);
                }
                ...
                logScript('Skip-load mode disabled');
              });
            }
          }
