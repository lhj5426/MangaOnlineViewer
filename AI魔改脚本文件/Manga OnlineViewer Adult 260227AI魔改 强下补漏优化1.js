// ==UserScript==
// @name          Manga OnlineViewer Adult 260227AI魔改 强下补漏优化1
// @author        Tago
        ...
        <div id="pageInputGroup">
          ...
          <div id="pageCalcGroup" style="margin-top: 10px; font-size: 14px; color: #666; display: flex; flex-direction: column; gap: 8px; align-items: center;">
            ...
            <div style="margin-top: 15px; padding: 12px; background: #e3f2fd; border: 2px solid #2196f3; border-radius: 8px; width: 100%;">
              <div id="batchCalculatorTitle" style="text-align: center; margin-bottom: 10px; font-weight: bold; color: #1976d2; font-size: 15px;">
                批次计算器
              </div>
          ...
          <div style="margin-top: 15px; padding: 15px; background: #fff3cd; border: 2px solid #ffc107; border-radius: 8px;">
            <div style="display: block;">
              ...
              <div style="margin-top: 12px; padding-top: 12px; border-top: 1px dashed #ffc107;">
                <div style="font-size: 13px; color: #856404; font-weight: bold; text-align: center; margin-bottom: 8px;">或直接粘贴页码序号</div>
                <div style="display: flex; align-items: stretch; gap: 8px;">
                  <input
                    type="text"
                    id="pastePageNumbers"
                    placeholder="例如: 15,24,36,78"
                    style="flex: 1; padding: 10px 12px; border: 2px solid #ffc107; border-radius: 6px; font-size: 14px; background: #fff; outline: none; color: #000000; font-weight: bold;"
                  />
                  <button
                    type="button"
                    id="pastePageNumbersBtn"
                    style="display: flex; align-items: center; justify-content: center; padding: 0 20px; background: #e91e63; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: bold; white-space: nowrap; transition: background 0.2s;"
                ...
                <div id="pasteStatus" style="font-size: 13px; text-align: center; display: none; margin-top: 8px; padding: 8px; border-radius: 4px; line-height: 1.6;"></div>
              </div>
            </div>
          </div>
        </div>
        ...
        if (inputSlider) {
          ...
          let updateBatchCalculator = function () {
            ...
            for (let i = 0; i < divide; i++) {
              ...
              if (batchStart <= end) {
                ...
                if (i === 0 && batchEnd < end) {
                  // 第1批次：绿色填入按钮
                  batchBtn = `<button class="batch-fill-btn" data-page="${batchEnd}" data-batch="${i + 1}" style="margin-left: 8px; padding: 6px 16px; background: #4CAF50; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 13px; font-weight: bold; transition: all 0.2s;">填入</button>`;
                } else if (i > 0) {
                  // 第2批次及以后：橙色填入按钮（自动填入当前批次起始页和页数-1）
                  const pageCount = batchEnd - batchStart;  // 当前批次的页数-1（用于"开始页+"）
                  batchBtn = `<button class="batch-autofill-btn" data-start="${batchStart}" data-count="${pageCount}" data-batch="${i + 1}" style="margin-left: 8px; padding: 6px 16px; background: #FF9800; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 13px; font-weight: bold; transition: all 0.2s;">填入</button>`;
                }
          ...
          if (pasteBtn && pasteInput) {
            ...
            pasteBtn.addEventListener('click', async function () {
              ...
              if (!inputText) {
                if (pasteStatus) {
                  pasteStatus.innerHTML = '<div style="color: #721c24; background: #f8d7da; padding: 8px; border-radius: 4px;">请先输入或粘贴页码</div>';
              ...
              // 解析逗号分隔的页码
              const content = inputText.replace(/[，]/g, ','); // 兼容中文逗号
              processFileContent(content, '粘贴输入');
            });
