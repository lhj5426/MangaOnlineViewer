// ==UserScript==
// @name          Manga OnlineViewer Adult 260616AI魔改 批次填入按钮加上序号2
// @author        Tago
          ...
          let updateBatchCalculator = function () {
            ...
            for (let i = 0; i < divide; i++) {
              ...
              if (batchStart <= end) {
                ...
                if (i === 0 && batchEnd < end) {
                  // 第1批次：绿色填入按钮
                  batchBtn = `<button class="batch-fill-btn" data-page="${batchEnd}" data-batch="${i + 1}" style="margin-left: 8px; padding: 6px 16px; background: #4CAF50; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 13px; font-weight: bold; transition: all 0.2s;">填入${i + 1}</button>`;
                } else if (i > 0) {
