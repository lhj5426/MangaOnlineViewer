// ==UserScript==
// @name          Manga OnlineViewer Adult 260123AI魔改 修复绅士漫画 修改默认颜色 删除多语言默认启用分隔符关闭缩略图
// @author        Tago
...
(function () {
  ...
  let zip;
  const getFilename = (index, total, ext) => {
    const pad = Math.max(4, Math.floor(Math.log10(total)) + 1);
    return `${(index + 1).toString().padStart(pad, '0')}.${ext.replace('jpeg', 'jpg')}`;
  };
  ...
  async function getImageData(img, index, array) {
    ...
    logScript(`[Image ${index}] originalUrl: ${originalUrl?.substring(0, 50)}, displaySrc: ${displaySrc?.substring(0, 50)}, checking cache...`);

    if (isBase64ImageUrl(src)) {
      return Promise.resolve({
        name: getFilename(index, array.length, getExtensionBase64(src)),
    ...
    if (imageBlobCache.has(src)) {
      ...
      return {
        name: getFilename(index, array.length, ext),
        data: buffer,
      };
    }
  ...
  async function generateZip() {
    ...
    for (let i = 0; i < images.length; i++) {
      try {
        const imgData = await getImageData(images[i], i, images);
        data.push(imgData);
