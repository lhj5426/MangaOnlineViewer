// ==UserScript==
// @name          Manga OnlineViewer Adult 251224AI魔改版
// @author        Tago
...
const startButton = "#StartMOV {\n    font-size: 1em;\n    color: #fff;\n    cursor: pointer;\n    margin: 10px 10px;\n    padding: 5px 11px;\n    text-align: center;\n    border: none;\n    background-size: 300% 100%;\n    border-radius: 50px;\n    transition: all 0.4s ease-in-out;\n    background-image: linear-gradient(to right, #667eea, #764ba2, #6b8dd6, #8e37d7);\n    box-shadow: 0 4px 15px 0 rgba(116, 79, 168, 0.75);\n    position: fixed;\n    right: 0;\n    left: 0;\n    bottom: 0;\n    z-index: 105000;\n    height: 8em;\n    min-height: 35px;\n    width: 150px;\n}\n\n#StartMOV:hover {\n    background-position: 100% 0;\n    transition: all 0.4s ease-in-out;\n}\n\n#StartMOV:focus {\n    outline: none;\n}\n";
  ...
  function setupMasonry() {
    ...
    // 检查是否有图片已加载（更严格的检查）
    const hasLoadedImages = masonryOriginalPages.some(page => {
      const img = page.querySelector('.PageImg');
      if (!img) return false;
      const src = img.getAttribute('src') || img.getAttribute('data-src');
      // 确保src不为空且不是占位符
      return src && src.length > 0 && !src.startsWith('data:image/gif') && src !== '';
    });
    
    // 如果没有图片或页面数为0，延迟执行
    if (!hasLoadedImages || masonryOriginalPages.length === 0) {
  ...
  function setupGrid() {
    ...
    // 检查是否有图片已加载
    const hasLoadedImages = gridOriginalPages.some(page => {
      const img = page.querySelector('.PageImg');
      if (!img) return false;
      const src = img.getAttribute('src') || img.getAttribute('data-src');
      return src && src.length > 0 && !src.startsWith('data:image/gif') && src !== '';
    ...
    // 如果没有图片或页面数为0，延迟执行
    if (!hasLoadedImages || gridOriginalPages.length === 0) {
    ...
    const options = {
      ...
      didOpen() {
        ...
        if (inputSlider) {
          ...
          const observerEvent = _.debounce(changedInput, 1000);
