// ==UserScript==
// @name          Manga OnlineViewer Adult 260607AI魔改 启动界面添加加载批次管理
// @author        Tago
...
(function () {
  ...
  function changeDefaultBatchDivide(event) {
    ...
    saveSettingsValue('defaultBatchDivide', Math.min(9, Math.max(2, divide)));
  }
  ...
  function addZip(img) {
    ...
    zip.file(img.name, img.data, {
      binary: true,
      createFolders: true,
      // Most image formats are already compressed; STORE is much faster than re-deflating them.
      compression: 'STORE',
    });
  }
  ...
  async function lateStart(site, begin = 1, existingManga = null) {
    ...
    const launchDefaultBatchDivide = Math.min(
      9,
      Math.max(2, parseInt(getSettingsValue('defaultBatchDivide') ?? 4, 10) || 4),
    );
      ...
      didOpen() {
        ...
        if (inputSlider) {
          ...
          let updateBatchCalculator = function () {
            ...
            const divide = Math.max(2, parsedDivide);
          ...
          launchBatchDivideSelect?.addEventListener('change', function () {
            ...
            if (Number.isNaN(divide)) return;
            const normalizedDivide = Math.min(9, Math.max(2, divide));
