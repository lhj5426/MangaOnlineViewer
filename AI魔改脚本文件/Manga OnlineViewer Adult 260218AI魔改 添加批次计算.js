// ==UserScript==
// @name          Manga OnlineViewer Adult 260218AI魔改 添加批次计算
// @author        Tago
...
(function () {
      ...
      if (
        ...
        setTimeout(
          async () => {
            ...
            // 用GM_xmlhttpRequest获取图片并缓存blob
            if (!isObjectURL(src) && !isBase64ImageUrl(src)) {
              logScript(`[addImg ${index}] Using GM_xmlhttpRequest to cache`);
              try {
                const blob = await new Promise((resolve, reject) => {
                  GM_xmlhttpRequest({
                    ...
                    onload(res) {
                      if (res.status === 200) {
                        imageBlobCache.set(src, res.response);
                        logScript(`[addImg ${index}] ✓ Cached blob: ${src.substring(0, 50)}, type: ${res.response.type}`);
                ...
                img.setAttribute('data-blob-cached', 'true'); // 标记blob已缓存
                src = blobUrl;
              } catch (e) {
                logScript(`[addImg ${index}] ✗ GM request failed: ${e.message}`);
                // 失败就用原URL
              ...
              logScript(`[addImg ${index}] Skipping cache, using src directly`);
      ...
      if (src && img) {
        ...
        if (!isObjectURL(src) && !isBase64ImageUrl(src)) {
          try {
            ...
            imageBlobCache.set(src, response.response);
            ...
            // 失败就用原URL
            logScript('Failed to fetch image via GM_xmlhttpRequest, using original URL:', e);
            img.setAttribute('data-blob-cached', 'false');
          }
        } else {
          img.setAttribute('data-blob-cached', 'true'); // base64或blob URL直接可用
  ...
  async function getImageData(img, index, array, maxPageNum) {
    ...
    logScript(`[Image ${index}] actualPageNum: ${actualPageNum}, originalUrl: ${originalUrl?.substring(0, 50)}, displaySrc: ${displaySrc?.substring(0, 50)}, checking cache...`);
    ...
    if (imageBlobCache.has(src)) {
      ...
      else ext = getExtension(src) || 'jpg';

      logScript(`[Image ${index}] ✓ Using blob cache, type: ${blob.type}, ext: ${ext}`);
      return {
    ...
    // 回退到Canvas方式（如果blob缓存没有）
    logScript(`[Image ${index}] ✗ Blob cache miss, using Canvas fallback`);
    return new Promise((resolve, reject) => {
      ...
      // 根据原图扩展名决定输出格式
      let ext = getExtension(src).toLowerCase() || 'png';
  ...
  function addZip(img) {
    logScript(`${img.name} Added to Zip`);
  ...
  async function generateZip() {
    ...
    images.forEach((img, i) => {
      ...
      logScript(`Image ${i}: src=${src.substring(0, 50)}, cached=${imageBlobCache.has(src)}`);
    });
    ...
    for (let i = 0; i < images.length; i++) {
      try {
        const imgData = await getImageData(images[i], i, images, maxPageNum);
        data.push(imgData);
        updateProgress(i + 1, '提取图片');
      } catch (e) {
    ...
    zip.file('标题.txt', title, { binary: false });
    ...
    // 生成zip文件
    document.getElementById('dlProgressText').textContent = '正在压缩...';
    zip.generateAsync(
      { type: 'blob' },
      (metadata) => {
        const percent = Math.round(metadata.percent);
        document.getElementById('dlProgressText').textContent = `压缩中... ${percent}%`;
        document.getElementById('dlProgressInner').style.width = `${percent}%`;
      }
    ).then(content => {
  ...
  async function generateZipPart() {
    ...
    for (let i = 0; i < images.length; i++) {
      try {
        ...
        logScript(`Failed to extract image ${i + 1}: ${e.message}`);
        // 获取实际页码用于错误报告
        let actualPageNum = i + 1;
        const pageContainer = images[i].closest('.MangaPage');
        if (pageContainer && pageContainer.id) {
          const match = pageContainer.id.match(/^Page(\d+)$/);
          if (match) {
            actualPageNum = parseInt(match[1], 10);
        ...
        failedIndices.push(actualPageNum);
      }
      updateProgress(i + 1, '提取图片');
    }
    ...
    if (failedIndices.length > 0) {
      zip.file('未加载图片列表.txt', '以下图片未成功加载 (可能还在加载中或加载失败):\n' + failedIndices.join('\n'), { binary: false });
    }
    ...
    const reflow = () => {
      ...
      masonryOriginalPages.forEach((page, idx) => {
        ...
        imgContainer.setAttribute('style',
          'width:100% !important;' +
          'padding-bottom:' + (aspectRatio * 100) + '% !important;' +
          'background-image:' + (src ? 'url("' + src.replace(/"/g, '\\"') + '")' : 'none') + ' !important;' +
        ...
        // 监听原始图片加载，同步更新背景图和宽高比
        const updateBg = () => {
          const newSrc = img.getAttribute('src') || img.getAttribute('data-src') || '';
          if (newSrc) {
            imgContainer.style.backgroundImage = 'url("' + newSrc.replace(/"/g, '\\"') + '")';
          }
          // 更新宽高比
          if (img.naturalWidth && img.naturalHeight) {
            const newRatio = img.naturalHeight / img.naturalWidth;
            imgContainer.style.paddingBottom = (newRatio * 100) + '%';
          }
        };
        img.addEventListener('load', updateBg);
        // 使用MutationObserver监听src变化
        const observer = new MutationObserver(updateBg);
        observer.observe(img, { attributes: true, attributeFilter: ['src', 'data-src'] });

        wrap.appendChild(imgContainer);

        // 辅助函数：从背景图或原始图片获取当前src
        const getCurrentSrc = () => {
        ...
        if (originalFunctions) {
          ...
          toolbar.querySelector('.Reload')?.addEventListener('click', (e) => {
            ...
            setTimeout(() => {
              const newSrc = page.querySelector('.PageImg')?.getAttribute('src') || src;
              imgContainer.style.backgroundImage = 'url("' + newSrc.replace(/"/g, '\\"') + '")';
            }, 500);
          });
  ...
  function setupGrid() {
    ...
    if (!controls) {
      ...
      style.textContent = `
        ...
        #Header:has(#GridControls) { flex-wrap:wrap !important; }
      `;
    ...
    const buildGrid = () => {
      ...
      imageInfos.forEach((item, idx) => {
        ...
        // 使用背景图片方式显示，避免创建新img元素导致重复加载
        const imgContainer = document.createElement('div');
        imgContainer.className = 'grid-img-container';
        const currentSrc = src || '';
        imgContainer.setAttribute('style',
          'width:100% !important;' +
          'height:100% !important;' +
          'background-image:' + (currentSrc ? 'url("' + currentSrc.replace(/"/g, '\\"') + '")' : 'none') + ' !important;' +
          'background-size:cover !important;' +
          'background-position:center !important;' +
          'background-repeat:no-repeat !important;'
        );
        ...
        wrap.appendChild(imgContainer);
        // 保存imgContainer引用用于后续获取src
        wrap._imgContainer = imgContainer;
        ...
        gridElements.push({ wrap, rowDiv, imgContainer, item });
      });
  ...
  async function lateStart(site, begin = 1) {
    ...
    const options = {
      ...
      html: html`
        <!-- 规则范围和视图模式选择 -->
        <div id="launchSettings" style="margin-bottom: 15px; padding: 10px; background: #f5f5f5; border-radius: 8px;">
        ...
        <div id="pageInputGroup">
          ...
          <div id="pageCount" style="margin-top: 10px; font-size: 14px; color: #666;">
            已选择: <span id="selectedCount" style="font-weight: bold; color: #2196f3;">${endPage - beginPage + 1}</span> 张图片
          </div>
          <div id="pageCalcGroup" style="margin-top: 10px; font-size: 14px; color: #666; display: flex; flex-direction: column; gap: 8px; align-items: center;">
            <!-- 快捷预设按钮 -->
            ...
            <!-- 批次计算功能 -->
            <div style="margin-top: 15px; padding: 12px; background: #e3f2fd; border: 2px solid #2196f3; border-radius: 8px; width: 100%;">
              <div style="text-align: center; margin-bottom: 10px; font-weight: bold; color: #1976d2; font-size: 15px;">
                批次计算器
              </div>
              <div style="display: flex; align-items: center; gap: 5px; justify-content: center; margin-bottom: 8px;">
              ...
              <div id="batchInfo" style="margin-top: 10px; padding: 10px; background: #fff; border-radius: 6px; font-size: 13px; line-height: 1.8;">
                <div style="color: #1976d2; font-weight: bold; margin-bottom: 5px;">批次信息：</div>
            ...
            <div style="display: block;">
              ...
              <div style="display: flex; align-items: stretch; gap: 8px;">
                <input
                  ...
                  placeholder="点击选择文件..."
                  readonly
                  style="flex: 1; padding: 10px 12px; border: 2px solid #ffc107; border-radius: 6px; font-size: 14px; background: #fff; outline: none; color: #000000; font-weight: bold; cursor: pointer;"
                />
                <label style="display: flex; align-items: center; justify-content: center; padding: 0 20px; background: #6c757d; color: white; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: bold; white-space: nowrap; transition: background 0.2s;">
                  浏览
                  <input
                    type="file"
              ...
              <div id="importStatus" style="font-size: 13px; text-align: center; display: none; margin-top: 10px; padding: 8px; border-radius: 4px; line-height: 1.6;">
              </div>
              <div style="font-size: 12px; color: #856404; text-align: center; font-style: italic; margin-top: 10px; line-height: 1.5;">
                导入txt文件后，将只加载文件中列出的页面<br/>
                <span style="font-size: 11px; color: #999;">留空则使用下方的页面范围设置</span>
              </div>
            </div>
          </div>
        </div>
      ...
      didOpen() {
        ...
        if (inputSlider) {
          ...
          let updateBatchCalculator = function() {
            ...
            const divide = parseInt(batchDivideInput.value, 10) || 2;
            ...
            for (let i = 0; i < divide; i++) {
              ...
              if (batchStart <= end) {
                // 只有第1批次显示填入按钮，其他批次显示复制按钮
                let batchBtn = '';
                if (i === 0 && batchEnd < end) {
                  // 第1批次：填入按钮
                  batchBtn = `<button class="batch-fill-btn" data-page="${batchEnd}" style="margin-left: 8px; padding: 6px 16px; background: #4CAF50; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 13px; font-weight: bold; transition: all 0.2s;">填入</button>`;
                } else if (i > 0) {
                  // 第2批次及以后：复制页数按钮（紫色，用于"开始页+"输入框）
                  const batchPageCount = batchEnd - batchStart;
                  batchBtn = `<button class="batch-copy-pagecount-btn" data-count="${batchPageCount}" style="margin-left: 8px; padding: 6px 16px; background: #9C27B0; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 13px; font-weight: bold; transition: all 0.2s;">复制</button>`;
                }
                
                // 第2批次及以后：复制尾页按钮（橙色，复制结束页，用于"结束页"输入框）
                let copyEndPageBtn = '';
                if (i > 0) {
                  copyEndPageBtn = `<button class="batch-copy-endpage-btn" data-page="${batchEnd}" style="margin-left: 8px; padding: 6px 16px; background: #FF9800; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 13px; font-weight: bold; transition: all 0.2s;">复制尾页</button>`;
                }
                
                batchHTML += `<div style="margin-bottom: 5px;">第 ${i + 1} 批次: ${batchStart} - ${batchEnd} (共 ${batchEnd - batchStart + 1} 页) ${batchBtn}${copyEndPageBtn}</div>`;
                
                // 只有当前批次结束页小于总结束页时，才显示下一批次起始页（带复制按钮）
                const nextStart = batchEnd + 1;
                if (batchEnd < end && nextStart <= end) {
                  const nextCopyBtn = `<button class="batch-copy-btn" data-page="${nextStart}" style="margin-left: 8px; padding: 6px 16px; background: #2196F3; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 13px; font-weight: bold; transition: all 0.2s;">复制</button>`;
                  batchHTML += `<div style="margin-left: 20px; color: #ff6b6b; font-weight: bold; margin-bottom: 8px;">→ 下批次起始页: ${nextStart} ${nextCopyBtn}</div>`;
                }
              }
            }
            ...
            // 为填入按钮添加点击事件
            document.querySelectorAll('.batch-fill-btn').forEach(btn => {
              btn.addEventListener('click', function() {
                const endPage = parseInt(this.getAttribute('data-page'), 10);
                const currentBegin = parseInt(pageBeginInput?.value ?? '0', 10) || beginPage;
                
                if (pageAddInput && !isNaN(endPage) && !isNaN(currentBegin)) {
                  // 计算需要加的数量：结束页 - 开始页
            ...
            // 为复制页数按钮添加点击事件（用于"开始页+"输入框）
            document.querySelectorAll('.batch-copy-pagecount-btn').forEach(btn => {
              btn.addEventListener('click', function() {
            ...
            // 为复制尾页按钮添加点击事件（复制结束页，用于"结束页"输入框）
            document.querySelectorAll('.batch-copy-endpage-btn').forEach(btn => {
              btn.addEventListener('click', function() {
                const page = this.getAttribute('data-page');
                if (page) {
                  navigator.clipboard.writeText(page).then(() => {
                    // 视觉反馈
                    this.textContent = '✓已复制';
                    this.style.background = '#e68900';
                    setTimeout(() => {
                      this.textContent = '复制尾页';
          ...
          if (filePathInput) {
            ...
            filePathInput.placeholder = '点击此处或点击浏览按钮选择文件...';
          }
