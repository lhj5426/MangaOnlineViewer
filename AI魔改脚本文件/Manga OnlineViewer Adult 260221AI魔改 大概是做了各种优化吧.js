﻿// ==UserScript==
// @name          Manga OnlineViewer Adult 260221AI魔改 大概是做了各种优化吧
// @author        Tago
...
(function () {
          ...
          setTimeout(async () => {
            ...
            if (image.img.parentElement) {
              ...
              img.onload = function() {
                ...
                thumb?.classList.remove('imgBroken');
                if (thumb) {
                  thumb.setAttribute('src', this.getAttribute('src') ?? '');
                }
  ...
  async function getImageData(img, index, array, maxPageNum) {
    ...
    if (isObjectURL(displaySrc)) {
      try {
        ...
        else ext = getExtension(originalUrl || src) || 'jpg';
  ...
  const SettingsPanelLoading = () => loadMode() + loadSpeed() + maxConcurrentLoads();
